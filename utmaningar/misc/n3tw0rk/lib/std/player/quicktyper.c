...
#define DEFAULT_ALIASES ([ "la":"look at", "us":"upstream", "ds":"downstream" ])
...
static private mapping m_alias_list = DEFAULT_ALIASES;
static private mapping m_nick_list = ([ ]);
/*
 * Prototypes.
 */
...
modify_command(string str)
{
    ...
    /* Resolve for aliases. */
    if (mappingp(m_alias_list) && m_alias_list[words[0]])
    {
    ...
    /* Resolve nicknames. */
    if (m_sizeof(m_nick_list) && (words[0] != "unnick") && (words[0] != "nick"))
    {
