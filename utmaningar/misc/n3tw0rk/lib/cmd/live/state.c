...
show_subloc(string subloc, object on, object for_obj)
{
    ...
    {
        res_yes = capitalize(on->query_pronoun()) + " consists ";
        res_no = capitalize(on->query_pronoun()) + " seems ";
    }

    if (strlen(tmp = on->show_subloc_data(on, for_obj)))
    {
        res = res_yes;
        res += "of the following data:\n";
        res += tmp;
    }    
    else
    {
        res = res_no;
        res += "to be devoid of any content.";
    }
...
varargs int show_status(string str)
{
    if (str)
    {
        notify_fail("Status command takes no arguments.\n");
        return 0;
    }

    write("You are " + LANG_ADDART(TP->QN) + ", roaming the network.\n");
    write(show_subloc("blah",TP,TP));
    return 1;
}

int filter_layers(object ob, int layer)
{
    return (ob->query_layer() == layer);
}
...
headers(string str)
{
    ...
    case "datalink":
        show_layer = 1;
        write("== Layer 1 - Physical/datalink layer ==\n");
        break;
    ...
    case "transport":
        show_layer = 3;
        write("=== Layer 3 - Transportation layer ====\n");
        break;
    ...
    case "application":
        show_layer = 4;
        write("====== Layer 4 - Application layer ====\n");
        break;
    ...
    if(show_layer < 1)
    {
        write("For which layer (1 - 4, data or all) would you want to list your headers?\n");
        return 1;
    }

    object *obs = filter(all_inventory(TP),&filter_layers(,show_layer));
    if(!sizeof(obs))
    {
        if(show_layer == 5)
            write(TP->format_data() + "\n");
        else
            write("No data.\n");
    }
    else
        write(obs[0]->format_data() + "\n");
    return 1; 
}
