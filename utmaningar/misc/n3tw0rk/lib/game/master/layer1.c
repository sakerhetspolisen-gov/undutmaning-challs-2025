...
int block_me()
{
    ...
    say(C(get_admin_name()) + " stops " + TP->QN + " from going upwards.\n");
    return 1;
}
...
int up_check()
{
   ...
   if(TP->always_ok_to_leave())
    ...
    if(!check_mac(hdr->get_field_value(1)))
    {
    ...
    {
        tell_room(TO,C(get_admin_name()) + " says: Sorry, but your indetification is not in order!\n");
        return block_me();
    }
...
void block_leave_message(string direction)
{
    write(C(get_admin_name()) + " looks you over and stops you from leaving "+direction+".\n");
...
int downstream_check()
{
    ...
    if(!hdr)
    {
        block_leave_message("downstream");
        return 1;
    }
